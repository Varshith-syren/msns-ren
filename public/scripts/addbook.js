...
submitButton.addEventListener('click',()=>{
    ...
    fetch('https://msns-soi.onrender.com/admin/addbook',{body:JSON.stringify(newBook),method:"POST",headers: { 'Content-Type': 'application/json' }}).then((res)=>{return res.json()})
    .then((response)=>{
        if(response.status=='fail'){
            ...
            setTimeout(
                ()=>{
                    window.location.href="https://msns-soi.onrender.com/admin"
                },3100
            );
