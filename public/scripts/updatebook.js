...
deleteButtons.forEach((x)=>{
    x.addEventListener('click',async ()=>{
        fetch('https://msns-soi.onrender.com/getbook/'+x.classList[1]).then((res)=>{return res.json()}).then((book)=>{
        ...
        await fetch('https://msns-soi.onrender.com/deletebook/'+x.classList[1]);
...
const openUpdateBookPopUpOverlay=(book,bookid)=>{
    ...
    document.querySelector('#admin-addbook-update-button').addEventListener('click',()=>{
        ...
        fetch('https://msns-soi.onrender.com/admin/updatebook',{body:JSON.stringify(newbook),method:"POST",headers: { 'Content-Type': 'application/json' }}).then((res)=>{return res.json()})
    .then((response)=>{
        if(response.status=='fail'){
            ...
            /*location.reload().then(()=>{
                ...
                setTimeout(()=>{
                    //window.location.href='https://msns-soi.onrender.com/admin/search/#search-bookDisplay'
                },200);
